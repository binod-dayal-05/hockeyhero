package model;

import java.util.Calendar;
import java.util.Date;

// reconfigured from https://github.students.cs.ubc.ca/CPSC210/AlarmSystem

//Represents a Hockey Hero event.
public class Event {
    private static final int HASH_CONSTANT = 13;
    private Date dateLogged;
    private String description;

    //EFFECTS: creates event with given descriotion and gets current date/time stamp
    public Event(String description) {
        dateLogged = Calendar.getInstance().getTime();
        this.description = description;
    }

    //EFFECTS: returns date of the event with time
    public Date getDate() {
        return dateLogged;
    }

    //EFFECTS: returns description of event
    public String getDescription() {
        return description;
    }

    //EFFECTS: override equals
    @Override
    public boolean equals(Object other) {
        if (other == null) {
            return false;
        }
            

        if (other.getClass() != this.getClass()) {
            return false;
        }

        Event otherEvent = (Event) other;

        return (this.dateLogged.equals(otherEvent.dateLogged) 
                && this.description.equals(otherEvent.description));
    }

    //EFFECTS: override hashcode
    @Override
    public int hashCode() {
        return (HASH_CONSTANT * dateLogged.hashCode() + description.hashCode());
    }

    @Override
    public String toString() {
        return dateLogged.toString() + "\n" + description;
    }
}
